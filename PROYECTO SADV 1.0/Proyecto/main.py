import sys,time, random, locale, ast, re, os
from EnviarCorreo import mandarCorreoFactura, mandarCorreoHtml
from Conexion import (obtenerProductos,enviarCompra,obtenerCodigosActualizacion,enviarCodigos,obtenerInventario,obtenerCodigosParaVender,
                    verificarCodigos,enviarVenta,obtenerUsuarios,borrarUs,agregarUs)
...
class VentanaIngreso(QMainWindow):
    def __init__(self):
        ...
        self.ImagenLabel.setPixmap(QtGui.QPixmap("Imagenes/" + str(random.randrange(1, 15)) + ".jpg"))
        ...
        self.comboboxTipoCuenta.addItems(["Administrador/a", "Empleado/a"])
    ...
    def abrirInventario(self):
        ...
        Tipo_cuenta = True if (self.comboboxTipoCuenta.currentText() == "Administrador/a") else False
...
class VentanaInventario(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Ancho columnas
        for indice, ancho in enumerate((70, 85, 70, 530, 80, 154), start=0):
    ...
    def abrirCompras(self):
        self.hide()
        otraVentana = VentanaCompras(self)
    ...
    def abrirVentas(self):
        self.hide()
        otraVentana = VentanaVentas(self)
        otraVentana.Admin = self.Admin
        otraVentana.ver()
        otraVentana.show()
    ...
    def abrirUsuarios(self):
        self.hide()
        otraVentana = VentanaUsuarios(self)
        otraVentana.show()
...
class VentanaCompras(QMainWindow):
    ...
    def __init__(self, parent=None):
        super(VentanaCompras, self).__init__(parent)
        loadUi('UI/Compras.ui', self)
        self.setWindowIcon(QIcon("Imagenes/Logo.png"))
...
class VentanaVentas(QMainWindow):
    ...
    def abrirContinuarVenta(self):
        ...
        if (identificacion != "" and correo != ""):
            otraVentana = VentanaContinuarVentas(self)
            otraVentana.Admin = self.Admin
            self.hide()
            otraVentana.show()
...
class VentanaContinuarVentas(QMainWindow):
    Admin = False

    def __init__(self, parent=None):
        super(VentanaContinuarVentas, self).__init__(parent)
